...
io.on('connection', (socket) => {
  ...
  socket.on('join_doctor',    (doctorId) => { socket.join(`doctor:${doctorId}`); socket.join(`user:${doctorId}`); });
...
// Cron jobs (only run in long-lived local/server process)
...
cron.schedule('0 * * * *', async () => {
  try {
    ...
    const appts = await Appointment.find({ date: today, status: { $in: ['pending', 'confirmed'] }, reminderSent: { $ne: true } })
    ...
    for (const a of appts) {
      ...
      if (diff > 0 && diff <= 3600000) {
        ...
        queueEmail(a.patient?.email, `⏰ Reminder: Appointment in ~${Math.round(diff / 60000)} min`,
...
cron.schedule('*/30 8-20 * * *', async () => {
  try {
    ...
    const candidates = await Appointment.find({ status: { $in: ['pending', 'confirmed'] }, date: today })
      .populate('patient', 'name email').populate('doctor', 'name');
    let count = 0;
  ...
  try {
    ...
    for (const cap of caps) {
      ...
      if (pct > 90) {
        ...
        const n = await Notification.create({ user: cap.doctor, title: '😓 Fatigue Alert', message: `You saw ${pct}% capacity today.`, type: 'fatigue_alert' });
